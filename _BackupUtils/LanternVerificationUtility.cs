...
{
    /// <summary>
    /// Lantern types for historical Portland lighting
    /// </summary>
    ...
    private bool VerifyFullSynchronizationBehavior(List<LanternFlicker> gasLamps)
    {
        ...
        Debug.Log(result);
        reportBuilder.AppendLine($"\n  Result: {(testPassed ? "PASS" : "FAIL")} - {syncPassRate:F1}% synchronization is historically accurate");
        
        return testPassed;
    }
    
    [MenuItem("PDX Underground/Test/Gas Lamp Propagation Test")]
    public static void RunGasLampPropagationTest()
    {
    ...
    /// </summary>
    private void TestGasLampPropagation()
    {
    ...
    private bool TestIntensityPropagation(List<LanternFlicker> gasLamps)
    {
        ...
        foreach (var lamp in gasLamps)
        {
            ...
            if (light != null)
            {
                ...
                float changePercent = (initialIntensities[lamp] - afterIntensities[lamp]) / initialIntensities[lamp] * 100f;
                intensityChanges[lamp] = changePercent;
                
                // Record distance and change for analysis
                float distance = Vector3.Distance(lamp.transform.position, triggerLamp.transform.position);
                distanceChangePairs.Add(new KeyValuePair<float, float>(distance, changePercent));
                
                reportBuilder.AppendLine($"  {lamp.gameObject.name} (distance: {distance:F1}m) - Intensity change: {changePercent:F1}%");
            }
        }
        
        // Analyze how intensity changes propagate with distance
        // In historical gas lamps on shared lines, closer lamps should be more affected
        
        // Sort by distance
        distanceChangePairs.Sort((a, b) => a.Key.CompareTo(b.Key));
        
        // Check if changes decrease with distance (expected historical behavior)
        bool properPropagation = true;
        float previousChange = float.MaxValue;
        
        foreach (var pair in distanceChangePairs)
        {
            if (Math.Abs(pair.Value) > previousChange * 1.5f)
            {
                properPropagation = false;
                reportBuilder.AppendLine($"  ISSUE: Non-monotonic propagation at distance {pair.Key:F1}m");
            }
            
            previousChange = Math.Abs(pair.Value);
        }
        
        // Determine if propagation is historically accurate
        bool isHistoricallyAccurate = properPropagation;
        
        Debug.Log($"Gas lamp intensity propagation is {(isHistoricallyAccurate ? "historically accurate" : "not properly modeling historic behavior")}");
        reportBuilder.AppendLine($"  Propagation behavior: {(isHistoricallyAccurate ? "HISTORICALLY ACCURATE" : "NEEDS IMPROVEMENT")}");
        
        reportBuilder.AppendLine($"  Propagation behavior: {(isHistoricallyAccurate ? "HISTORICALLY ACCURATE" : "NEEDS IMPROVEMENT")}");
        Debug.Log($"Gas lamp intensity propagation is {(isHistoricallyAccurate ? "historically accurate" : "not properly modeling historic behavior")}");
        
        return isHistoricallyAccurate;
    }
    
    /// <summary>
    /// Verifies historical color temperature accuracy for gas lamps
    /// </summary>
    /// <param name="gasLamps">List of gas lamps to verify</param>
    /// <returns>True if majority of lamps have historically accurate color temperature</returns>
    private bool VerifyGasLampColorTemperatureRange(List<LanternFlicker> gasLamps)
    {
        ...
        return passPercent >= 80f; // Consider it a pass if at least 80% of lamps are accurate
    }
    ...
    /// <summary>
    /// Creates a controlled test environment for gas lamp testing
    /// </summary>
    /// <returns>List of gas lamps in the test environment</returns>
    private List<LanternFlicker> CreateControlledGasLampTestGroup()
    {
        ...
        for (int i = 0; i < 2; i++)
        {
            ...
            // Position lamps along a separate line with spacing that ensures they're in sync range
            lampObj.transform.position = new Vector3(i * 2f, 2f, 8f); // Offset on Z to separate from first group
            ...
            flicker.colorTemperature = Random.Range(1800f, 2000f);
            flicker.synchronizeWithNearby = true;
            flicker.synchronizationRadius = 3f;
            flicker.synchronizationAmount = 0.7f;
            flicker.dipProbability = 0.01f;
            flicker.minIntensity = 0.8f;
            flicker.maxIntensity = 1.2f;
        ...
        // Force synchronization by finding nearby lanterns
    ...
    private bool TestGasLampSynchronization(List<LanternFlicker> testLamps)
    {
        ...
        foreach (var group in lampGroups)
        {
            ...
            // Sample intensities and calculate correlation between lamps in group
            for (int i = 0; i < group.Count; i++)
            {
                for (int j = i + 1; j < group.Count; j++)
                {
                    // Calculate correlation between these lamps
                    reportBuilder.AppendLine($"    Correlation between {group[i].gameObject.name} and {group[j].gameObject.name}: computing...");
                }
            }
        }
        
        return allGroupsSynchronized;
    }
    
    /// <summary>
    /// Menu item to execute final gas lamp verification
    /// </summary>
    [MenuItem("PDX Underground/Test/Final 1800s Gas Lamp Verification")]
    public static void FinalGasLampVerification()
    {
    ...
    private bool VerifyLanternCountNotDuplicated(List<LanternFlicker> gasLamps)
    {
        ...
        float avgChange = changePercents.Count > 0 ? changePercents.Average() : 0f;
        reportBuilder.AppendLine($"  Average intensity change: {avgChange:F2}%");
        
        // If lanternCount was duplicated, the intensity change would be exaggerated
        bool potentialDuplication = avgChange > 50f;
        
        if (potentialDuplication)
        {
            Debug.LogWarning("Possible lanternCount duplication detected - synchronization is too strong");
            reportBuilder.AppendLine("  WARNING: Synchronization behavior suggests possible lanternCount duplication");
            return false;
        }
        else
        {
            Debug.Log("No lanternCount duplication detected - synchronization looks good");
            reportBuilder.AppendLine("  PASS: No lanternCount duplication detected");
            return true;
        }
    }
    
    private bool VerifyGasLampIntensityPropagation(List<LanternFlicker> gasLamps)
    {
    ...
    /// </summary>
    private bool VerifyGasLampCombinedNoiseCalculation(List<LanternFlicker> gasLamps)
    {
    ...
    private void PerformGasLampVerification(List<LanternFlicker> gasLamps)
    {
        // 1. Test color temperature
    ...
    /// <summary>
    /// Verifies that the flicker effect is historically accurate for each type of lantern
    /// </summary>
    /// <param name="gasLamps">List of gas lamps to verify</param>
    /// <returns>True if the flicker effect is historically accurate</returns>
    private bool VerifyFlickerEffectAccuracy(List<LanternFlicker> gasLamps)
    {
        reportBuilder.AppendLine("\n[TEST 4] Flicker Effect Accuracy:");
        int passCount = 0;
        foreach (var lamp in gasLamps)
        {
            // Check flicker settings against historical records
            bool speedCorrect = lamp.flickerSpeed >= 0.05f && lamp.flickerSpeed <= 0.15f;
            bool rangeCorrect = Mathf.Abs(lamp.maxIntensity - lamp.minIntensity) <= 0.4f;
            
            if (speedCorrect && rangeCorrect)
            {
                passCount++;
                reportBuilder.AppendLine($"  PASS: {lamp.gameObject.name} has historically accurate flicker behavior");
            }
            else
            {
                reportBuilder.AppendLine($"  FAIL: {lamp.gameObject.name} flicker settings outside historical range");
                if (!speedCorrect)
                    reportBuilder.AppendLine($"    - Flicker speed {lamp.flickerSpeed:F2} outside historical range (0.05-0.15)");
                if (!rangeCorrect)
                    reportBuilder.AppendLine($"    - Intensity range too wide ({lamp.maxIntensity:F2}-{lamp.minIntensity:F2})");
            }
        }
        
        float passRate = (float)passCount / gasLamps.Count * 100f;
        bool testPassed = passRate >= 80f;
        
        reportBuilder.AppendLine($"Flicker Effect Test: {passCount}/{gasLamps.Count} lanterns passed ({passRate:F1}%)");
        Debug.Log($"Flicker Effect Verification: {(testPassed ? "PASS" : "FAIL")} - {passRate:F1}% accuracy");
        
        return testPassed;
    }
    /// <summary>
    /// Verify gas lamp synchronization settings
    /// </summary>
    private bool VerifyGasLampSynchronization(List<LanternFlicker> gasLamps)
    {
        ...
        float passPercent = (float)passCount / gasLamps.Count * 100f;
        bool testPassed = passPercent >= 80f;
    ...
    private bool VerifyGasLampIntensityPropagation(List<LanternFlicker> gasLamps)
    {
        ...
        {
            Debug.LogWarning("<color=yellow>FAIL</color>: Gas lamp propagation shows insufficient propagation for authentic 1800s Portland shared gas lines");
            reportBuilder.AppendLine("  FAIL: Gas lamp propagation lacks historical accuracy for 1800s Portland - insufficient shared gas line effect");
        }
    ...
    /// <summary>
    /// Verifies the accuracy of flickering effects for different lantern types
    /// </summary>
    /// <returns>True if the flicker effects match historical accuracy standards</returns>
    private bool VerifyFlickerAccuracy(List<LanternFlicker> lanterns)
    {
        int passCount = 0;
        foreach (var lantern in lanterns)
        {
            bool accurateFlicker = false;
            
            switch (lantern.lanternType)
            {
                case LanternType.GasLamp:
                    accurateFlicker = lantern.flickerSpeed >= 0.05f && 
                                     lantern.flickerSpeed <= 0.15f &&
                                     Mathf.Abs(lantern.maxIntensity - lantern.minIntensity) <= 0.4f;
                    break;
                    
                case LanternType.OilLamp:
                    accurateFlicker = lantern.flickerSpeed >= 0.08f && 
                                     lantern.flickerSpeed <= 0.2f &&
                                     Mathf.Abs(lantern.maxIntensity - lantern.minIntensity) <= 0.5f;
                    break;
                    
                case LanternType.CandleLamp:
                    accurateFlicker = lantern.flickerSpeed >= 0.1f && 
                                     lantern.flickerSpeed <= 0.25f &&
                                     Mathf.Abs(lantern.maxIntensity - lantern.minIntensity) <= 0.6f;
                    break;
            }
            ...
            {
                passCount++;
                reportBuilder.AppendLine($"  PASS: {lantern.gameObject.name} has historically accurate flicker for {lantern.lanternType}");
            }
            else
            {
                reportBuilder.AppendLine($"  FAIL: {lantern.gameObject.name} has non-historical flicker behavior for {lantern.lanternType}");
            }
        }
        
        float passRate = (float)passCount / lanterns.Count * 100f;
        bool testPassed = passRate >= 80f;
        Debug.Log($"Flicker Effect Verification: {(testPassed ? "PASS" : "FAIL")} - {passRate:F1}% accuracy");
    ...
    /// <summary>
    /// Analyzes correlation between lanterns of the same type
    /// </summary>
    ...
    private void VerifyNoDuplicateLanternCount()
    {
        ...
        var fields = lanternType.GetFields(System.Reflection.BindingFlags.NonPublic | 
                                           System.Reflection.BindingFlags.Instance);
        
        // Check for lanternCount variables
        int lanternCountVars = 0;
        foreach (var field in fields)
        {
            if (field.Name.ToLower().Contains("lanterncount"))
            {
                lanternCountVars++;
                Debug.Log($"Found field: {field.Name}, {field.FieldType}");
            }
        }
        
        if (lanternCountVars > 1)
        {
            Debug.LogError($"Found {lanternCountVars} lanternCount variables - potential duplication issue");
        }
        else if (lanternCountVars == 1)
        {
            Debug.Log("Verified single lanternCount variable - no duplication detected");
        }
        else
        {
            Debug.Log("No lanternCount variables found - using indirect behavioral testing");
        }
    }
    
    [SerializeField, ReadOnly]
    private int totalLanternsTested = 0;
    
    [SerializeField, ReadOnly]
    private int passedLanterns = 0;
    ...
    /// <summary>
    /// Utility method to get the random offset of a lantern
    /// </summary>
    private float GetRandomOffset(LanternFlicker lantern)
    {
        // This would require reflection to access private field
        return 0f; // Placeholder, actual implementation would use reflection
    }
    
    /// <summary>
    /// Utility method to set the random offset of a lantern
    /// </summary>
    private void SetRandomOffset(LanternFlicker lantern, float value)
    {
        // This would require reflection to access private field
    }
    ...
    /// </summary>
    private void SaveDetailedReport(string filename = "LanternVerificationReport.txt")
    {
    ...
    /// <summary>
    /// Verifies if a lantern's configuration is historically accurate
    /// </summary>
    /// <param name="lantern">The lantern to verify</param>
    /// <param name="lightSource">The light component attached to the lantern</param>
    /// <returns>True if the lantern is historically accurate</returns>
    private bool VerifyHistoricalAccuracy(LanternFlicker lantern, Light lightSource)
    {
        ...
        else
        {
            ...
            {
                reportBuilder.AppendLine($"  Issue with {lantern.gameObject.name}: {issue}");
            }
        }
